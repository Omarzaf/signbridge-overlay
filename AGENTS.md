...
**If `START.md` exists in the repository root, read it first.** It names which
agent you are and which workstream you own in this worktree, then points at your
full brief in `docs/agent-briefs.md`. It is untracked, per-worktree scaffolding
generated by `tools/agent-start.mjs`; it never overrides this contract.

If `START.md` is absent you are in the primary checkout, not an assigned
worktree. Do not begin implementation work here — ask which worktree to use.
